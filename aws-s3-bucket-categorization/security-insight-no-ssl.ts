...
const OUTPUT_EXCEL = path.join(OUTPUT_DIR, "s3-no-ssl-findings.xlsx");
...
async function exportNoSslBucketsToExcel(): Promise<void> {
  ...
  const noSslRows: noSSLBucketRows[] = []; // "No-SSL Buckets"
  ...
  const wsNoSsl = workbook.addWorksheet("S3.5 - No-SSL Buckets");
  wsNoSsl.columns = [
  ...
  console.log(`  - "No-SSL Buckets" sheet rows: ${noSslRows.length}`);
...
(async () => {
  try {
    ...
    console.error("Failed to export S3 No-SSL buckets to Excel:", err);
    process.exit(1);
  }
})();
